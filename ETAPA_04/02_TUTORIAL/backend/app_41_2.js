...
app.post('/insereUsuario', urlencodedParser, (req, res) => {
	...
	" VALUES (?,?,?)";
	sql2 = "INSERT INTO cargo (userId,nome_cargo) " + 
		" VALUES (?,?)";
	console.log(sql);
	db.run(sql, [req.body.nome,req.body.email,req.body.telefone],  function (err) {
		console.log(err);
		if (err) {
			throw err;
		} else{
			idRefKey = this.lastID.valueOf();
			console.log("Ultimo AutoIncremento da tabela Usuario: "+ idRefKey);
			console.log(sql2);
			console.log("idRefKey antes de inserir na tabela cargo: "+ idRefKey);
			db.run(sql2, [idRefKey,req.body.cargo], function(err) {
				if (err) {
					throw err;
				}	
			});
	...
	res.write('<p>USUARIO INSERIDO COM SUCESSO! no ID: </p>  ' + idRefKey);
	res.write('<p>Siga aqui: </p><a href="/">VOLTAR</a>');
